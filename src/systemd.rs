...
pub fn install(args: InstallArgs) -> Result<()> {
    info!("Installing systemd user service and socket on port {}", args.port);
...
pub fn get_systemd_listeners() -> Result<Vec<std::os::unix::io::RawFd>> {
    ...
    let fds: Vec<RawFd> = (SD_LISTEN_FDS_START..SD_LISTEN_FDS_START + listen_fds)
        .collect();
    
    debug!("Got {} file descriptors from systemd: {:?}", listen_fds, fds);
    Ok(fds)
}
